...
class RainbowChannel extends React.Component {
  ...
  applySearch(dateVideoArray) {
    if (
      this.props.currentSearch == undefined ||
    ...
    let terms = this.props.currentSearch
      .split(" ")
      .filter((s) => s.length > 0)
      .map((s) => s.toLowerCase());
  ...
  getFilteredVideoArray() {
    ...
    if (this.props.videoArray == []) {
      return (
        <Text style={styles.emptySearch}>
          No videos in this channel. Check back later
        </Text>
      );
    }
    var dateVideoArray = [];
    ...
    if (options.length <= 0) {
      return (
        <Text style={styles.emptySearch}>No videos match your search</Text>
      );
    }
